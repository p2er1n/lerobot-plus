...
def rollout(
    ...
    actions_save_path: str | None = None,
) -> dict:
    ...
    if actions_save_path is not None:
        actions_np = actions_tensor.cpu().numpy()
        np.save(actions_save_path, actions_np)
    if return_observations:
...
def eval_policy(
    ...
    actions_save_dir: Path | None = None,
) -> dict:
    """
    Args:
        env: The batch of environments.
    ...
    for batch_ix in progbar:
        ...
        # Build save path per batch
        batch_actions_path = None
        if actions_save_dir is not None and seeds is not None:
            batch_actions_path = str(actions_save_dir / f"batch_{batch_ix}.npy")
        rollout_data = rollout(
            ...
            actions_save_path=batch_actions_path,
        )
...
def eval_main(cfg: EvalPipelineConfig):
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type) if cfg.policy.use_amp else nullcontext():
        info = eval_policy_all(
            ...
            actions_dir=Path(cfg.output_dir) / "actions" if os.environ.get("LEROBOT_SAVE_ACTIONS", "0") == "1" else None,
        )
...
def eval_one(
    ...
    *,
    ...
    task_result = eval_policy(
        ...
        actions_save_dir=actions_save_dir,
    )
...
def run_one(
    ...
    *,
    ...
    if actions_save_dir is not None:
        task_actions_dir = actions_save_dir / f"{task_group}_{task_id}"
        task_actions_dir.mkdir(parents=True, exist_ok=True)
    ...
    metrics = eval_one(
        ...
        return_episode_data=return_episode_data,
        start_seed=start_seed,
        actions_save_dir=task_actions_dir,
    )
...
def main():
    import sys
    save_val = [a.split("=", 1)[1] for a in sys.argv if a.startswith("--save-actions=")]
    if save_val:
        os.environ["LEROBOT_SAVE_ACTIONS"] = "1" if save_val[0].lower() in ("true", "1", "yes") else "0"
    sys.argv = [a for a in sys.argv if not a.startswith("--save-actions=")]
    init_logging()
